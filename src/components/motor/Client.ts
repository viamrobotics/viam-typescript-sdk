...
export class MotorClient implements Motor {
  ...
  async setPower(power: number, extra = {}) {
    ...
    const response = await promisify<
      motorApi.SetPowerRequest,
      motorApi.SetPowerResponse
    >(motorService.setPower.bind(motorService), request)
    return response
  }
  ...
  async goFor(rpm: number, revolutions: number, extra = {}) {
    ...
    request.setExtra(googleProtobufStructPb.Struct.fromJavaScript(extra))
    const response = await promisify<
      motorApi.GoForRequest,
      motorApi.GoForResponse
    >(motorService.goFor.bind(motorService), request)
    return response
  }
  ...
  async goTo(rpm: number, positionRevolutions: number, extra = {}) {
    ...
    request.setExtra(googleProtobufStructPb.Struct.fromJavaScript(extra))
    const response = await promisify<
      motorApi.GoToRequest,
      motorApi.GoToResponse
    >(motorService.goTo.bind(motorService), request)
    return response
  }
  ...
  async motorStop(extra = {}) {
    ...
    request.setExtra(googleProtobufStructPb.Struct.fromJavaScript(extra))
    const response = await promisify<
      motorApi.StopRequest,
      motorApi.StopResponse
    >(motorService.stop.bind(motorService), request)
    return response
  }
