...
> extends BaseStream {
  ...
  protected startRequest(signal?: AbortSignal) {
    ...
    try {
      ...
      console.error('error writing headers', error); // eslint-disable-line no-console
  ...
  protected resetStream() {
    ...
    try {
      ...
      console.error('error writing reset', error); // eslint-disable-line no-console
      this.closeWithRecvError();
    }
  }
  ...
  protected writeMessage(eos: boolean, msgBytes?: Uint8Array) {
    try {
      ...
      console.error('error writing message', error); // eslint-disable-line no-console
      this.closeWithRecvError();
    }
  }
  ...
  public onResponse(resp: Response) {
    switch (resp.type.case) {
      ...
      default: {
        console.error('unknown response type', resp.type.case); // eslint-disable-line no-console
        break;
      }
    }
  }
