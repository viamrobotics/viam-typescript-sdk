...
Syncs a specific release from viamrobotics/viam-typescript-sdk to Jira RSDK project
...
# Configuration
GITHUB_REPO = "viamrobotics/viam-typescript-sdk"
...
def create_jira_version(github_version):
    """Create version in Jira RSDK project with format 'Typescript SDK v{version}'"""
    # Format as "Typescript SDK v{version}"
    jira_version_name = f"Typescript SDK v{github_version}"
    ...
    payload = {
        ...
        "released": False,
        "description": f"Release v{github_version} from {GITHUB_REPO}",
    }
...
def main(version_input):
    ...
    print(f"📦 Creating Jira version: Typescript SDK v{version}")
    ...
    for ticket_key in ticket_keys:
        ...
        if status == "Awaiting Release":
            ...
            if set_fix_version_and_close(ticket_key, version_id):
                print(f"   ✅ {ticket_key} closed with fix version Typescript SDK v{version}")
                closed_count += 1
            else:
            ...
            if set_fix_version(ticket_key, version_id):
                print(f"   ✅ {ticket_key} tagged with fix version Typescript SDK v{version}")
                tagged_count += 1
            else:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print("Examples:")
        print("  python sync_jira_release.py 0.68.0")
        print("  python sync_jira_release.py v0.68.0")
        sys.exit(1)
